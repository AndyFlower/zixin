...
Spring使得创建一个企业级java应用变的简单。 It provides everything you need to embrace the Java language in an enterprise environment, with support for Groovy and Kotlin as alternative languages on the JVM, and with the flexibility to create many kinds of architectures depending on an application’s needs. 对于``Spring``框架的5.1以上，需要``JDK8``以上版本 ，并且也支持``JDK11 LTS``开箱即用。建议javase8更新60作为java8的最小补丁版本。 Java SE 8 update 60 is suggested as the minimum patch release for Java 8,但是通常建议去使用最近的补丁版本 .
